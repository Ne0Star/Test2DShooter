...
public class PlayerController : Player
{
    ...
    public float jumpAnimationHeight, jumpAnimationSpeed;
    private IEnumerator Jump()
    {
        Vector2 lastPos = header.transform.localPosition;
        while (header.transform.localPosition.y < jumpAnimationHeight)
        {
            header.transform.localPosition = Vector3.Lerp(header.transform.localPosition, new Vector3(header.transform.localPosition.x, jumpAnimationHeight), Time.fixedDeltaTime * jumpAnimationSpeed);
            yield return new WaitForFixedUpdate();
        }
        if (header.transform.localPosition.y >= jumpAnimationHeight)
            body.AddForce(new Vector2(body.velocity.x, currentControllerData.jumpPower), ForceMode2D.Impulse);
        header.transform.localPosition = lastPos;
        yield break;
    }
}
